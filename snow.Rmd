...
```{r}
data<- fread("https://www.inscc.utah.edu/~steenburgh/ml/gfslccforecast_2023100312.txt", blank.lines.skip = T)
...
#geom_ribbon(aes(y = mean, ymin = mean -se, ymax = mean+se, fill = well), alpha = .3) +
  ...
  scale_x_datetime(date_breaks = '12 hour', date_labels = "%m/%d %H", minor_breaks = "12 hour")+
