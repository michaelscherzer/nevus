...
```
```{r}
theme_rjt <- function (base_size = 12, base_family = "Times New Roman") {
...
#geom_ribbon(aes(y = mean, ymin = mean -se, ymax = mean+se, fill = well), alpha = .3) +
  ...
  scale_x_datetime(date_breaks = '18 hour', date_labels = "%b%d-%I%p")+
...
#geom_ribbon(aes(y = mean, ymin = mean -se, ymax = mean+se, fill = well), alpha = .3) +
  ...
  #scale_color_manual(values = my_colors) + scale_fill_manual(values = my_colors)+
   ggtitle("Snow Predication at Alta, scraped from https://www.inscc.utah.edu/") +theme(plot.title = element_text(hjust = 0.5))+
  scale_x_datetime(date_breaks = '6 hour', date_labels = "%a %b %d %I%p", expand=c(0,1), limits = breaks)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))+
  theme(plot.margin = margin(1, 2, 1, 1, "cm"))


p+scale_y_continuous(breaks = seq(0, snow_max+5, 5), 
                   limits = c(0,snow_max+1), 
                   expand = c(.1,1))->p

p
coord_cartesian(expand = T, clip = "on", xlim = c(min+1, max+1))

ggsave(filename = "snow.JPG", dpi = "retina", width = 16, height = 12,plot = p)
```
